...
local filefunc, err = loadfile(".vlware/apis/comgr")
...
function readConfig(cfg)
	local file = fs.open("/.vlware/sys.conf", "r")
	if not file then
		return nil
	end
	local lines = {}
...
for _, v in pairs(systemDirs) do
	if not fs.exists(v) then
		...
		print("Reinstall VLWARE OS or run 'updater' to fix this problem.")
		fgSet(colors.white)
		return
	end
end
...
if fs.exists("/.vlware/connor.sys") then
	print("Loading connor.sys...")
	shell.run("/.vlware/connor.sys")
end
...
if not fs.exists("/.vlware/sys.conf") then
	...
	local file = fs.open("/.vlware/sys.conf", "w")
...
elseif config.read("/.vlware/tim.conf", "upd8") then
	shell.run("/usr/bin/changelog")
	local x = config.list()
	x['upd8'] = false
	for i,v in pairs(x) do
		config.write("/.vlware/tim.conf",i,v)
	end
end
os.version = function()
	return config.read(nil, "oem") or "VLWARE OS 1.1"
end
shell.setDir("")
if fs.exists("/.vlware/oemboot") then
	shell.run("/.vlware/oemboot")
end
shell.run("/.vlware/login")
if fs.exists("/.vlware/oemstart") then
	shell.run("/.vlware/oemstart")	
end
	...
	bgmgr.addProcess(function()
		...
		while true do
			...
			if e == "syslog" and msg then
				local logfile = fs.open(".vlware/sys.log", "a")
