...
if not fs.exists("/.vlware/users.db") then
	local file = fs.open("/.vlware/users.db", "w")
	clear()
	print("Welcome to VLWARE OS!")
...
-- load .vlware/users.db
local file = fs.open("/.vlware/users.db", "r")
